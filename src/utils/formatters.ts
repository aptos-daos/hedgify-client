...
const getTicker = (str: string) => {
  const arr = str.replace(/[^\w\s]/g, "").replace("-", "").split(" ");
  if (arr.length > 2) {
    return arr
      .map((w) => w[0])
      .join("")
 ...
 * @param seconds - Number of seconds (Unix timestamp in seconds)
 * @returns Date object
 */
const secondsToDate = (seconds: number | string): Date => {
  if (typeof seconds === "string") return secondsToDate(Number(seconds));
